...
while (True):
  # 클라이언트로부터 메세지 수신 부분
  # 메세지 수신 후 50% 확률로 응답하지 않고 50% 확률로 'ack' 응답 메세지를 클라이언트로 전송
  ...
  # 클라이언트로 메세지 전송 부분
  # 'ack' 응답 메세지를 수신하지 못하는 경우, 2초 간격으로 최대 5회 재전송 (총 6회)
  msg = input('-> ');
  reTx = 0;
  while (reTx <= 5):
    response = str(reTx) + ' ' + msg;
    udp_socket.sendto(response.encode('utf-8'), address);
    udp_socket.settimeout(2) # 2초 간격 재전송을 위한 소켓의 timeout 설정
